...
fn main() {
    ...
    // horiz and ver products
    for i in 0..20 {
        for j in 0..=20 - 4 {
            let row_prod = a.slice(s![i, j..j + 4]).product();
            let col_prod = a.slice(s![j..j + 4, i]).product();
            max_product = max(col_prod, max(max_product, row_prod));
        }
    }
    ...
    for i in 0..=20 - 4 {
        for j in 0..=20 - 4 {
            let diag_prod = a[[i, j]] * a[[i + 1, j + 1]] * a[[i + 2, j + 2]] * a[[i + 3, j + 3]];

            let adiag_prod = a[[i + 3, j]] * a[[i + 2, j + 1]] * a[[i + 1, j + 2]] * a[[i, j + 3]];

            max_product = max(adiag_prod, max(diag_prod, max_product));
        }
    }
